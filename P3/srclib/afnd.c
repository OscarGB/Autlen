...
AFND * NuevoDesde2Afnd1O(AFND * p_afnd1, AFND * p_afnd2){
	AFND * nuevo = NULL;
	char nombre1[10], nombre2[10];
	char ** simbolos1 = alfabetoGetTodosSimbolos(p_afnd1->alfabeto);
	char ** simbolos2 = alfabetoGetTodosSimbolos(p_afnd2->alfabeto);
	char * simbolo_actual, * nombre_aux;
	int i = 0, j = 0, k = 0;
	int * transiciones;
	int repetidos = 0;
	...
	for(i = 0; i < p_afnd1->num_estados; i++){
		sprintf(nombre1, "q%d", indiceaux++);
		AFNDInsertaEstado(nuevo, nombre1, NORMAL);
	}
	...
	for(i = 0; i < p_afnd1->num_estados; i++){
		nombre_aux = estadoNombre(nuevo->estados[i]);
		for(j = 0; j < p_afnd1->num_simbolos; j++){
			simbolo_actual = alfabetoGetSimbolo(p_afnd1->alfabeto, j);
			transiciones = getTransicion(p_afnd1->estados[i], j);
			for(k = 0; k < p_afnd1->num_estados; k++){
				if(transiciones[k] == 1){
					AFNDInsertaTransicion(nuevo, nombre_aux, simbolo_actual, estadoNombre(nuevo->estados[k]));
				}
			}
		}
	}
	...
	/*Insertamos las transiciones del primer automata*/
	for(i = 0; i < p_afnd2->num_estados; i++){
		nombre_aux = estadoNombre(nuevo->estados[i+p_afnd1->num_estados]);
		for(j = 0; j < p_afnd2->num_simbolos; j++){
			...
			for(k = 0; k < p_afnd2->num_estados; k++){
				if(transiciones[k] == 1){
					AFNDInsertaTransicion(nuevo, nombre_aux, simbolo_actual, estadoNombre(nuevo->estados[k+p_afnd1->num_estados]));
				}
			}
		}
	}
...
AFND * AFND1OUne(AFND * p_afnd1O_1, AFND * p_afnd1O_2){
	...
	nuevo = NuevoDesde2Afnd1O(p_afnd1, p_afnd2);
